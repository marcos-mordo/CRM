...
export function CompaniesTable({ companies, customFields = { fields: [], valuesByRow: {} } }: { companies: Row[]; customFields?: { fields: { key: string; label: string }[]; valuesByRow: Record<string, Record<string, string>> } }) {
  ...
  const sortAccessors: Record<string, (c: Row) => any> = {
    ...
    revenue: (c) => (c.annualRevenue != null ? Number(c.annualRevenue) : null),
    ...Object.fromEntries(customFields.fields.map((f) => [`cf_${f.key}`, (c: Row) => customFields.valuesByRow[c.id]?.[f.key]])),
  };
  ...
  return (
    <>
      ...
      <div className="p-4 border-b flex items-center justify-between gap-3">
        ...
        {filtered.length !== companies.length && filtered.length > 0 && (
          <FilteredExportButton entity="companies" ids={filtered.map((c) => c.id)} count={filtered.length} />
        )}
